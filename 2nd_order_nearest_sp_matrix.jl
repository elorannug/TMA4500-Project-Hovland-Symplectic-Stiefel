...
# ╔═╡ 765f5af5-ca3f-4c6a-81f0-adba6db4aeeb
# For saving to .csv
...
and then transforming it to a symplectic matrix by applying the Cayley transform" [(source)](A random point on ``\mathrm{Sp}(2n)`` is constructed by generating a
random Hamiltonian matrix ``Ω \in \mathfrak{sp}(2n,F)`` with norm `σ`,
...
We don't really have an expression for 'directional_r_grad()`, so this section is turned off at the moment. 

JZ p. 15: *"We consider R–TR in two variants: R-TR1 uses the true Riemannian Hessian, and
R–TR2 uses the approximation given in (2.25). We will write R–TR when addressing
both variants."*
"""
...
# ╔═╡ 3c56ea70-c3c1-4f08-b544-07adc5be764d
begin
	...
	# Save to a CSV file
	#CSV.write("2nd_order_nearest_sp_matrix_test_n100k5.csv", data)
end
