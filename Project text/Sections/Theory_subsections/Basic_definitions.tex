...
\todo[inline]{Note that even though the elements of the symplectic group and symplectic stiefel manifold are matrices, the word "point" will be used to refer to a specific matrix, as it is a point on the matrix manifold.}
...
\begin{definition}[Horizontal \& Vertical Space]\label{def:horizontal_vertical_space}
    ...
    \end{equation*}
    \cite[p.~4]{JensenZimmermann2024}named the \textup{vertical space} at $\overline{p}$. Note that Canonically chosen as the orthogonal complement of $\mathcal{V}_{\overline{p}}$ in $T_{\overline{p}}\overline{\mathcal{M}}$, the \textup{horizontal space} \cite[p.~48]{AbsilMahonySepulchre2008} is defined as %  p.48
    \begin{equation*}
