...
One component to many optimization algorithms is the gradient. In the Euclidean setting the gradient of the cost function is a vector that points in the direction of the steepest ascent. Analogously, the Riemannian gradient points in the direction of the steepest ascent on the manifold. We define the notion of a Riemannian gradient rigorously in Definition~\ref{def:riemannian_gradient}. In this section we will state the Riemannian gradient explicitly, and prove that it satisfies necessary properties. 
%A useful analogy of what this means is to consider the sphere, and some cost function defining a hilly surface on the sphere. The riemannian gradient would then point in the direction towards the closest highest point, whereas the taking the negative of the riemannian gradient would point towards the closest valley floor.
\begin{proposition}
    Given a function $f\colon \mathrm{SpSt}(2n, 2k)\xrightarrow{}\mathbb{R}$, the \text{Riemannian gradient} (as in Definition~\ref{def:riemannian_gradient}) with respect to $g_{p}$ is given by
