...
\begin{proposition}
    Given \todo{add general def of R grad.?}a function $f:\mathrm{SpSt}(2n, 2k)\xrightarrow{}\mathbb{R}$, the \text{Riemannian gradient} with respect to $g_{p}$ is given by
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    where the last two terms cancel after applying \eqref{eq:J_2n_properties}, and the tangent space condition \todo{add ref to Tspspt}ref ??, $p ^{\mathrm{T}}JX=-X ^{\mathrm{T}}Jp$.
\end{proof}