...
\end{equation*}
where the notion of quotient manifold is as in Definition \ref{def:quotient_manifold}. It has dimension $\mathrm{dim}(\mathrm{SpSt}(2n, 2k))=(2n-2k+1)k$. 
...
The expression for the tangent space follows straightforwardly from the definition of $\mathrm{SpSt}(2n, 2k)$. Assume we have a curve, $c(t)\in \mathrm{SpSt}(2n,2k)$, s.t. $c(0)=p$ and $\dot{c}(0)\coloneqq\tfrac{d}{dt}c(t)|_{t=0}=X$. Since $c(t)$ is a curve in $\mathrm{SpSt}(2n,2k)$, by \eqref{eq:spst_def} it must satisfy the following condition:
